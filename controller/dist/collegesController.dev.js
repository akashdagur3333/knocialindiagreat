...
var _require = require('../module/colleges'),
    Colleges = _require.Colleges;

var counterSchema = require('../module/counter');

var addColleges = function addColleges(req, res) {
  var req = req.body;
  var statusClg;
  ...
  counterSchema.findOneAndUpdate({
    ...
    colleges.save(function (err, docs) {
      if (!err) {
        res.json(docs);
      } else {
        res.json(err);
...
var getAllColleges = function getAllColleges(req, res) {
  Colleges.find(function (err, docs) {
    if (!err) {
      res.json(docs);
    } else {
      res.json(err);
...
var deleteColleges = function deleteColleges(req, res) {
  var deleteid = req.params.id;
  Colleges.findByIdAndDelete(deleteid, function (err, del) {
    if (!err) {
      res.json(del);
    } else {
      res.json(err);
...
var updateColleges = function updateColleges(req, res) {
  var req1 = req.body;
...
var addPanel = function addPanel(req, res) {
  var req1 = req.body;
  Colleges.findByIdAndUpdate(req.params.id, {
    tnp_head_name: req1.tnp_head_name,
    tnp_head_contact: req1.tnp_head_contact,
